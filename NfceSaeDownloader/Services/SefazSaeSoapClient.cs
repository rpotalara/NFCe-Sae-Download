...
namespace NfceSaeDownloader.Services
{
    public sealed class SefazSaeSoapClient
    {
        ...
        {
            var payload = new XElement(XName.Get("nfceListagemChaves", _config.ListagemNamespace),
                new XElement(XName.Get("versao", _config.ListagemNamespace), _config.VersaoLayout),
                new XElement(XName.Get("tpAmb", _config.ListagemNamespace), (int)_config.Ambiente),
                new XElement(XName.Get("dataHoraInicial", _config.ListagemNamespace), dataHoraInicial.ToString("yyyy-MM-ddTHH:mm")));

            if (dataHoraFinal.HasValue)
            {
                payload.Add(new XElement(XName.Get("dataHoraFinal", _config.ListagemNamespace), dataHoraFinal.Value.ToString("yyyy-MM-ddTHH:mm")));
            }

            var envelope = BuildSoapEnvelope(payload);
            var xml = PostSoap(_config.UrlListagem, _config.ListagemSoapAction, envelope, _config.Certificado);
            return ParseListagem(xml);
        }

        public NfceDownloadResponse DownloadXml(string chave)
        {
            var payload = new XElement(XName.Get("nfceDownloadXML", _config.DownloadNamespace),
                new XElement(XName.Get("versao", _config.DownloadNamespace), _config.VersaoLayout),
                new XElement(XName.Get("tpAmb", _config.DownloadNamespace), (int)_config.Ambiente),
                new XElement(XName.Get("chNFCe", _config.DownloadNamespace), chave));

            var envelope = BuildSoapEnvelope(payload);
            var xml = PostSoap(_config.UrlDownload, _config.DownloadSoapAction, envelope, _config.Certificado);
